...
weightedUTR_cell <- function(
    ...
    my_annot <- data.frame(annotation_gr) %>%
        ...
        dplyr::mutate(end_CDS = dplyr::if_else(strand == "-",
                                               min(start[which(type == "CDS")]),
                                               max(end[which(type == "CDS")]))) %>%
    ...
    my_expr <- tibble::as_tibble(data.frame(seurat_obj[[assay]]$counts), rownames = "gene_tr") %>%
        ...
        reshape2::melt(id.vars = c('gene_tr', 'gene_name', 'transcript_name')) %>%
        suppressWarnings() %>%
    ...
    # Integrate Seurat metadata and calculate weighted UTR length per cell
    my_expr <- tibble::as_tibble(seurat_obj[[group.by]], rownames = 'variable') %>%
        ...
        dplyr::distinct(variable, condition_defined, wUTRlength.cell) %>%
        data.table::data.table() %>%
        dplyr::rename(cluster = 'condition_defined')
